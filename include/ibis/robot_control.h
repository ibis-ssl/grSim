...
inline void theta_control(float target_theta, accel_vector_t * acc_vel, output_t * output, imu_t * imu)
{
    // PID
...
inline void accel_control(accel_vector_t * acc_vel, output_t * output, target_t * target, omni_t * omni)
{
...
inline void speed_control(accel_vector_t * acc_vel, output_t * output, target_t * target, imu_t * imu, omni_t * omni)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        float odom_diff_max = (float)OUTPUT_OUTPUT_LIMIT_ODOM_DIFF / OUTPUT_GAIN_ODOM_DIFF_KP;
