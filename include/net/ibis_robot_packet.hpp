...
namespace crane {
inline auto convertFloatToTwoByte(float val, float range)
...
inline auto convertTwoByteToFloat(uint8_t byte_high, uint8_t byte_low, float range)
    -> float {
...
#define FLOAT_FROM_2BYTE(name, range)                                                          \
  packet.name = convertTwoByteToFloat(                                                         \
    data[static_cast<int>(Address::name##_HIGH)], data[static_cast<int>(Address::name##_LOW)], range)
...
#define FLOAT_FROM_1BYTE(name, range) \
  packet.name = data[static_cast<int>(Address::name)] / 255.0f * range
    ...
    packet.DRIBBLE_POWER =
        data[static_cast<int>(Address::DRIBBLE_POWER)] / 20.0f;
...
inline RobotCommand::operator RobotCommandSerialized() const {
...
#define FLOAT_TO_1BYTE(name, range)                                      \
  ...
  serialized.data[static_cast<int>(RobotCommandSerialized::Address::CHECK)] =
      CHECK;
  ...
  serialized
      .data[static_cast<int>(RobotCommandSerialized::Address::DRIBBLE_POWER)] =
      static_cast<uint8_t>(DRIBBLE_POWER * 20);
  serialized
      .data[static_cast<int>(RobotCommandSerialized::Address::KICK_POWER)] =
      [&]() -> uint8_t {
    if (CHIP_ENABLE) {
  ...
  serialized
      .data[static_cast<int>(RobotCommandSerialized::Address::LOCAL_FLAGS)] =
      local_flags;
