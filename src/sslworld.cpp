...
SSLWorld::SSLWorld(QGLWidget *parent, ConfigWidget *_cfg,
                   RobotsFormation *form1, RobotsFormation *form2)
    : QObject(parent) {
  isGLEnabled = true;
  customDT = -1;
  _w = this;
  ...
  g->setViewpoint(0, -(cfg->Field_Width() + cfg->Field_Margin() * 2.0f) / 2.0f,
                  3, 90, -45, 0);
  p = new PWorld(0.05, 9.81f, g, cfg->Robots_Count());
  ball = new PBall(0, 0, 0.5, cfg->BallRadius(), cfg->BallMass(), 1, 0.7, 0);

  ground =
      new PGround(cfg->Field_Rad(), cfg->Field_Length(), cfg->Field_Width(),
                  cfg->Field_Penalty_Depth(), cfg->Field_Penalty_Width(),
                  cfg->Field_Penalty_Point(), cfg->Field_Line_Width(), 0);
  ray = new PRay(50);

  ROBOT_BLUE_CHASSIS_COLOR =
      QColor(QString::fromStdString(_cfg->v_ColorRobotBlue->getString()));
  ROBOT_YELLOW_CHASSIS_COLOR =
      QColor(QString::fromStdString(_cfg->v_ColorRobotYellow->getString()));

  // Bounding walls

  const double thick = cfg->Wall_Thickness();
  const double increment =
      cfg->Field_Margin() + cfg->Field_Referee_Margin() + thick / 2;
  ...
  walls[1] = new PFixedBox(-thick / 2, -pos_y, pos_z, siz_x, thick, siz_z, tone,
                           tone, tone);

  walls[2] = new PFixedBox(pos_x, -thick / 2, pos_z, thick, siz_y, siz_z, tone,
                           tone, tone);

  walls[3] = new PFixedBox(-pos_x, thick / 2, pos_z, thick, siz_y, siz_z, tone,
                           tone, tone);
  ...
  const double gpos_x =
      (cfg->Field_Length() + gthick) / 2.0 + cfg->Goal_Depth();
  ...
  for (int k = 0; k < cfg->Robots_Count(); k++)
    robots[k + cfg->Robots_Count()] =
        new Robot(p, ball, cfg, form2->x[k], form2->y[k], ROBOT_START_Z(cfg),
  ...
  for (int k = 0; k < 2 * cfg->Robots_Count(); k++) {
    ...
    p->createSurface(robots[k]->kicker->box, ball)->surface =
        ballwithkicker.surface;
    ...
    for (int j = k + 1; j < 2 * cfg->Robots_Count(); j++) {
      if (k != j) {
        p->createSurface(
            robots[k]->dummy,
            robots[j]->dummy); // seams ode doesn't understand cylinder-cylinder
                               // contacts, so I used spheres
...
QImage *createBlob(char yb, int i, QImage **res,
                   const QColor &color = QColor(63, 63, 63)) {
...
void SSLWorld::glinit() {
  ...
  for (int i = 0; i < cfg->Robots_Count(); i++)
    g->loadTexture(
        createBlob('b', i, &robots[i]->img, ROBOT_BLUE_CHASSIS_COLOR));
  ...
  // wrong
  for (int i = 0; i < 6; i++) {
    g->loadTexture(new QImage(QString(":/sky/neg_%1")
                                  .arg(i % 3 == 0   ? 'x'
                                       : i % 3 == 1 ? 'y'
                                                    : 'z') +
                              QString(".png")));
    g->loadTexture(new QImage(QString(":/sky/pos_%1")
                                  .arg(i % 3 == 0   ? 'x'
                                       : i % 3 == 1 ? 'y'
                                                    : 'z') +
                              QString(".png")));
...
void SSLWorld::step(dReal dt) {
  ...
  if (selected == -2) {
    ...
    best_dist = (bx - xyz[0]) * (bx - xyz[0]) + (by - xyz[1]) * (by - xyz[1]) +
                (bz - xyz[2]) * (bz - xyz[2]);
  }
  for (int k = 0; k < cfg->Robots_Count() * 2; k++) {
    if (robots[k]->selected) {
      g->getViewpoint(xyz, hpr);
      dReal dist =
          (robots[k]->select_x - xyz[0]) * (robots[k]->select_x - xyz[0]) +
...
void SSLWorld::sendRobotStatus(Robots_Status &robotsPacket,
                               const QHostAddress &sender, int team) {
...
void SSLWorld::recvActions() {
  ...
  while (commandSocket->hasPendingDatagrams()) {
    ...
    if (datagram.isValid()) {
      grSimPacket.ParseFromArray(datagram.data().data(),
                                 datagram.data().size());

      if (grSimPacket.has_commands()) {
        int team = 0;
        if (grSimPacket.commands().has_isteamyellow() &&
            grSimPacket.commands().isteamyellow())
          team = 1;

        for (int i = 0; i < grSimPacket.commands().robot_commands_size(); i++) {
          if (!grSimPacket.commands().robot_commands(i).has_id())
            continue;
          ...
          if ((id < 0) || (id >= cfg->Robots_Count() * 2))
            continue;
          bool wheels = false;
          if (grSimPacket.commands().robot_commands(i).has_wheelsspeed() &&
              grSimPacket.commands().robot_commands(i).wheelsspeed()) {
            if (grSimPacket.commands().robot_commands(i).has_wheel1())
              robots[id]->setSpeed(
                  0, grSimPacket.commands().robot_commands(i).wheel1());
            if (grSimPacket.commands().robot_commands(i).has_wheel2())
              robots[id]->setSpeed(
                  1, grSimPacket.commands().robot_commands(i).wheel2());
            if (grSimPacket.commands().robot_commands(i).has_wheel3())
              robots[id]->setSpeed(
                  2, grSimPacket.commands().robot_commands(i).wheel3());
            if (grSimPacket.commands().robot_commands(i).has_wheel4())
              robots[id]->setSpeed(
                  3, grSimPacket.commands().robot_commands(i).wheel4());
            wheels = true;
          }

          if (!wheels) {
            dReal vx = 0;
            if (grSimPacket.commands().robot_commands(i).has_veltangent())
              vx = grSimPacket.commands().robot_commands(i).veltangent();
            dReal vy = 0;
            if (grSimPacket.commands().robot_commands(i).has_velnormal())
              vy = grSimPacket.commands().robot_commands(i).velnormal();
            dReal vw = 0;
            if (grSimPacket.commands().robot_commands(i).has_velangular())
              vw = grSimPacket.commands().robot_commands(i).velangular();
          ...
          int rolling = 0;
          if (grSimPacket.commands().robot_commands(i).has_spinner() &&
              grSimPacket.commands().robot_commands(i).spinner()) {
            rolling = 1;
          }
      ...
      if (grSimPacket.has_replacement()) {
        for (int i = 0; i < grSimPacket.replacement().robots_size(); i++) {
          int team = 0;
          if (grSimPacket.replacement().robots(i).has_yellowteam() &&
              grSimPacket.replacement().robots(i).yellowteam())
            team = 1;

          if (!grSimPacket.replacement().robots(i).has_id())
            continue;
          int k = grSimPacket.replacement().robots(i).id();
          dReal x = 0, y = 0, dir = 0;
          bool turn_on = true;
          if (grSimPacket.replacement().robots(i).has_x())
            x = grSimPacket.replacement().robots(i).x();
          if (grSimPacket.replacement().robots(i).has_y())
            y = grSimPacket.replacement().robots(i).y();
          if (grSimPacket.replacement().robots(i).has_dir())
            dir = grSimPacket.replacement().robots(i).dir();
          if (grSimPacket.replacement().robots(i).has_turnon())
            turn_on = grSimPacket.replacement().robots(i).turnon();
          int id = robotIndex(k, team);
          if ((id < 0) || (id >= cfg->Robots_Count() * 2))
            continue;
          robots[id]->setXY(x, y);
        ...
        if (grSimPacket.replacement().has_ball()) {
          ...
          if (grSimPacket.replacement().ball().has_x())
            x = grSimPacket.replacement().ball().x();
          if (grSimPacket.replacement().ball().has_y())
            y = grSimPacket.replacement().ball().y();
          if (grSimPacket.replacement().ball().has_vx())
            vx = grSimPacket.replacement().ball().vx();
          if (grSimPacket.replacement().ball().has_vy())
            vy = grSimPacket.replacement().ball().vy();
    ...
    for (int team = 0; team < 2; team++) {
      ...
      if (updateRobotStatus)
        sendRobotStatus(robotsPacket, datagram.senderAddress(), team);
    }
  }
}
  ...
  for (int i = 0; i < cfg->Robots_Count(); i++) {
    if (auto packet = ibisRobotCommunicator->receive(i)) {
      ...
      double omega = ibisRobotCommunicator->getOmega(
          robots[id]->getDir(), packet->TARGET_GLOBAL_THETA, last_dt, i);
      robots[id]->setSpeed(packet->VEL_LOCAL_SURGE, packet->VEL_LOCAL_SWAY,
                           omega);
      double kick_speed = packet->KICK_POWER * MAX_KICK_SPEED;
      robots[id]->kicker->kick(kick_speed,
                               packet->CHIP_ENABLE ? kick_speed : 0.0);
      // TODO: use dribble power as double value
      robots[id]->kicker->setRoller(packet->DRIBBLE_POWER > 0.0);
    }
  }
}
...
void SSLWorld::simControlSocketReady() {
  ...
  while (simControlSocket->hasPendingDatagrams()) {
    ...
    simulatorCommand.ParseFromArray(datagram.data().data(),
                                    datagram.data().size());
    ...
    simControlSocket->writeDatagram(buffer.data(), buffer.size(),
...
void SSLWorld::blueControlSocketReady() {
  ...
  while (blueControlSocket->hasPendingDatagrams()) {
    ...
    blueControlSocket->writeDatagram(buffer.data(), buffer.size(),
                                     datagram.senderAddress(),
                                     datagram.senderPort());
  }
}
...
void SSLWorld::yellowControlSocketReady() {
  ...
  while (yellowControlSocket->hasPendingDatagrams()) {
    ...
    QByteArray buffer(robotControlResponse.ByteSize(), 0);
    robotControlResponse.SerializeToArray(buffer.data(), buffer.size());
    yellowControlSocket->writeDatagram(buffer.data(), buffer.size(),
                                       datagram.senderAddress(),
                                       datagram.senderPort());
  }
}

void SSLWorld::processSimControl(const SimulatorCommand &simulatorCommand,
  ...
  if (simulatorCommand.has_control()) {
    ...
    for (const auto &teleBot : simulatorCommand.control().teleport_robot()) {
      int id =
          robotIndex(teleBot.id().id(), teleBot.id().team() == YELLOW ? 1 : 0);
      if (id < 0) {
        continue;
      }
...
void SSLWorld::processRobotSpec(const RobotSpecs &robotSpec) const {
  auto settings = robotSpec.id().team() == YELLOW ? &cfg->yellowSettings
                                                  : &cfg->blueSettings;
...
void SSLWorld::processRobotLimits(const RobotSpecs &robotSpec,
                                  RobotSettings *settings) {
  if (robotSpec.limits().has_acc_speedup_absolute_max()) {
    settings->AccSpeedupAbsoluteMax =
        robotSpec.limits().acc_speedup_absolute_max();
  }
  if (robotSpec.limits().has_acc_speedup_angular_max()) {
    settings->AccSpeedupAngularMax =
        robotSpec.limits().acc_speedup_angular_max();
  }
...
void SSLWorld::processTeleportRobot(const TeleportRobot &teleBot,
                                    Robot *robot) {
  ...
  if (teleBot.has_y())
    y = teleBot.y();
  if (teleBot.has_orientation())
    orientation = teleBot.orientation();
  if (teleBot.has_v_x())
    vx = teleBot.v_x();
  if (teleBot.has_v_y())
    vy = teleBot.v_y();
  if (teleBot.has_v_angular())
    vAngular = teleBot.v_angular();
...
void SSLWorld::processTeleportBall(SimulatorResponse &simulatorResponse,
                                   const TeleportBall &teleBall) const {
  dReal x = 0, y = 0, z = 0;
  ball->getBodyPosition(x, y, z);
  const auto vel_vec = dBodyGetLinearVel(ball->body);
  auto vx = vel_vec[0];
  ...
  if (teleBall.has_teleport_safely()) {
    simulatorResponse.add_errors()->set_code(
...
void SSLWorld::processRobotControl(const RobotControl &robotControl,
                                   RobotControlResponse &robotControlResponse,
                                   Team team) {
  for (const auto &robotCommand : robotControl.robot_commands()) {
    ...
    if (robotCommand.has_kick_speed() && robotCommand.kick_speed() > 0) {
      ...
      double limit = robotCommand.kick_angle() > 0
                         ? robotCfg.MaxChipKickSpeed
                         : robotCfg.MaxLinearKickSpeed;
      if (kickSpeed > limit) {
        kickSpeed = limit;
      }
    ...
    if (robotCommand.has_move_command()) {
      processMoveCommand(robotControlResponse, robotCommand.move_command(),
                         robot);
    }
...
bool SSLWorld::visibleInCam(int id, double x, double y) {
  ...
  id %= 4;
  if (id == 0) {
    if (x > -CAMERA_OVERLAP && y > -CAMERA_OVERLAP)
      return true;
  }
  if (id == 1) {
    if (x > -CAMERA_OVERLAP && y < CAMERA_OVERLAP)
...
QPair<float, float> SSLWorld::cameraPosition(int id) {
  ...
  // these positions must be kept synchronized with the quadrants in
  // visibleInCam
...
SSL_WrapperPacket *SSLWorld::generatePacket(int cam_id) {
  ...
  // send the geometry for every camera consecutively to provide the camera
  ...
  if (((sendGeomCount++) / 4) % (cfg->sendGeometryEvery() / 4) == 0) {
    ...
    chip_fixed_loss->set_damping_xy_first_hop(
        cfg->BallModelChipFixedLossDampingXyFirstHop());
    chip_fixed_loss->set_damping_xy_other_hops(
        cfg->BallModelChipFixedLossDampingXyOtherHops());
  ...
  if (!cfg->vanishing() || (rand0_1() > cfg->ball_vanishing())) {
    if (visibleInCam(cam_id, x, y)) {
      ...
      if (project_airborne) {
        output_z = qBound(0.0, output_z - BALL_RADIUS,
                          CONVUNIT(CAMERA_HEIGHT) * SCALING_LIMIT);
        auto camera_pos = cameraPosition(cam_id);
        ...
        output_x = (output_x - camera_x) * (camera_z / (camera_z - output_z)) +
                   camera_x;
        output_y = (output_y - camera_y) * (camera_z / (camera_z - output_z)) +
                   camera_y;
      }
  ...
  for (int i = 0; i < cfg->Robots_Count() * 2; i++) {
    ...
    const double vanishing =
        (is_blue) ? cfg->blue_team_vanishing() : cfg->yellow_team_vanishing();
    ...
    if (!cfg->vanishing() || (rand0_1() > vanishing)) {
      ...
      if (visibleInCam(cam_id, x, y)) {
        SSL_DetectionRobot *rob =
            (is_blue) ? pPacket->mutable_detection()->add_robots_blue()
        ...
        rob->set_orientation(normalizeAngle(randn_notrig(dir, dev_a)) * M_PI /
                             180.0f);
      }
    }
  }
...
void SSLWorld::addFieldLinesArcs(SSL_GeometryFieldSize *field) {
  ...
  addFieldLine(field, "TopTouchLine", kXMin - kLineThickness / 2, kYMax,
               kXMax + kLineThickness / 2, kYMax, kLineThickness);
  addFieldLine(field, "BottomTouchLine", kXMin - kLineThickness / 2, kYMin,
               kXMax + kLineThickness / 2, kYMin, kLineThickness);
  addFieldLine(field, "LeftGoalLine", kXMin, kYMin, kXMin, kYMax,
               kLineThickness);
  addFieldLine(field, "RightGoalLine", kXMax, kYMin, kXMax, kYMax,
               kLineThickness);
  ...
  addFieldLine(field, "LeftPenaltyStretch",
               kXMin + kPenaltyDepth - kLineThickness / 2, -kPenaltyWidth / 2,
               kXMin + kPenaltyDepth - kLineThickness / 2, kPenaltyWidth / 2,
               kLineThickness);
  addFieldLine(field, "RightPenaltyStretch",
               kXMax - kPenaltyDepth + kLineThickness / 2, -kPenaltyWidth / 2,
               kXMax - kPenaltyDepth + kLineThickness / 2, kPenaltyWidth / 2,
               kLineThickness);

  addFieldLine(field, "RightGoalTopLine", kXMax, kGoalWidth / 2,
               kXMax + kGoalDepth, kGoalWidth / 2, kLineThickness);
  addFieldLine(field, "RightGoalBottomLine", kXMax, -kGoalWidth / 2,
               kXMax + kGoalDepth, -kGoalWidth / 2, kLineThickness);
  addFieldLine(field, "RightGoalDepthLine",
               kXMax + kGoalDepth - kLineThickness / 2, -kGoalWidth / 2,
               kXMax + kGoalDepth - kLineThickness / 2, kGoalWidth / 2,
               kLineThickness);

  addFieldLine(field, "LeftGoalTopLine", -kXMax, kGoalWidth / 2,
               -kXMax - kGoalDepth, kGoalWidth / 2, kLineThickness);
  addFieldLine(field, "LeftGoalBottomLine", -kXMax, -kGoalWidth / 2,
               -kXMax - kGoalDepth, -kGoalWidth / 2, kLineThickness);
  addFieldLine(field, "LeftGoalDepthLine",
               -kXMax - kGoalDepth + kLineThickness / 2, -kGoalWidth / 2,
               -kXMax - kGoalDepth + kLineThickness / 2, kGoalWidth / 2,
               kLineThickness);

  addFieldLine(field, "LeftFieldLeftPenaltyStretch", kXMin, kPenaltyWidth / 2,
               kXMin + kPenaltyDepth, kPenaltyWidth / 2, kLineThickness);
  addFieldLine(field, "LeftFieldRightPenaltyStretch", kXMin, -kPenaltyWidth / 2,
               kXMin + kPenaltyDepth, -kPenaltyWidth / 2, kLineThickness);
  addFieldLine(field, "RightFieldLeftPenaltyStretch", kXMax, -kPenaltyWidth / 2,
               kXMax - kPenaltyDepth, -kPenaltyWidth / 2, kLineThickness);
  addFieldLine(field, "RightFieldRightPenaltyStretch", kXMax, kPenaltyWidth / 2,
               kXMax - kPenaltyDepth, kPenaltyWidth / 2, kLineThickness);
...
RobotsFormation::RobotsFormation(E_FORMATION type, ConfigWidget *_cfg)
    : cfg(_cfg) {
  switch (type) {
  case FORMATION_OUTSIDE: {
    ...
    dReal teamPosX[MAX_ROBOT_COUNT] = {0.40, 0.80, 1.20, 1.60, 2.00, 2.40,
                                       ...
                                       0.40, 0.80, 1.20, 1.60};
    dReal teamPosY[MAX_ROBOT_COUNT] = {yv, yv, yv, yv, yv, yv, yv, yv,
  ...
  case FORMATION_OUTSIDE_FIELD: {
    double yv = -(_cfg->Field_Width() / 2 + _cfg->Field_Margin() +
                  _cfg->Field_Referee_Margin() + 0.5);
    dReal teamPosX[MAX_ROBOT_COUNT] = {0.40, 0.80, 1.20, 1.60, 2.00, 2.40,
                                       2.80, 3.20, 3.60, 4.00, 4.40, 4.80,
                                       0.40, 0.80, 1.20, 1.60};
    dReal teamPosY[MAX_ROBOT_COUNT] = {yv, yv, yv, yv, yv, yv, yv, yv,
 ...
 *
 * Example calls:
 * randn_notrig();	//returns normal deviate with mean=0.0, std.
 * deviation=1.0 randn_notrig(5.2,3.0);	//returns deviate with mean=5.2, std.
 * deviation=3.0
 *
 *
...
dReal rand0_1() { return (dReal)(rand()) / (dReal)(RAND_MAX); }
